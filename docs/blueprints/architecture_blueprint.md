...
3. **실전 모멘텀 및 매물대(POC) 필터 (AND 결합)**:
   ...
   - **볼륨 프로파일 POC 필터**: 최근 200봉 기준 최대 매물대(POC)를 계산한 후 종가 바로 위(`Price ~ Price * 1.02`)에 POC 저항이 있으면 LONG 차단, 종가 바로 아래(`Price * 0.98 ~ Price`)에 POC 지지가 있으면 SHORT 차단. (Bins 해상도는 ATR 변동성에 비례하여 30~200개로 동적 결정).
...
- **경과 봉 수 카운팅**: `process_closed_kline` 은 설정된 메인 캔들봉이 마감될 때마다 `pending_entries` 에 들어있는 모든 심볼의 `bars_elapsed`를 `1`씩 증가시킵니다.
- **상태 검증 및 3봉 타임아웃**:
  ...
  - **부분 체결 처리**: 취소 후 최종 체결 수량(filled)이 존재할 때, 부분 체결된 수량의 평가 금액(수량 * 현재가)이 **최소 주문 금액(5 USDT) 미만**일 경우 TP/SL 주문을 거는 대신 **즉시 해당 부분 체결분을 시장가 청산(`market_close`)** 하여 포지션 방치 리스크를 완전히 예방하며, 5 USDT 이상일 경우에 한하여 활성 포지션으로 편입하고 비례하여 TP/SL 주문을 발주합니다. **(긴급 청산 주문 발송 자체가 거래소 API 에러나 자산 부족 등으로 실패하는 경우, 텔레그램 메신저로 긴급 경보를 발송하여 관리자의 즉각적인 수동 청산을 유도합니다.)**
