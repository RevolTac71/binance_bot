...
- **상태 검증 및 3봉 타임아웃**:
  ...
  - **부분 체결 처리**: 취소 후 최종 체결 수량(filled)이 존재할 때, 부분 체결된 수량의 평가 금액(수량 * 현재가)이 **최소 주문 금액(5 USDT) 미만**일 경우 TP/SL 주문을 거는 대신 **즉시 해당 부분 체결분을 시장가 청산(`market_close`)** 하여 포지션 방치 리스크를 완전히 예방하며, 5 USDT 이상일 경우에 한하여 활성 포지션으로 편입하고 비례하여 TP/SL 주문을 발주합니다.
