...
* **POC 산출**: 최근 200개 캔들의 고가 최댓값과 저가 최솟값 범위를 변동성(\(ATR_{20} \times 0.05\)) 단위에 따라 Bins의 개수를 동적(최소 30개에서 최대 200개 해상도로 클리핑)으로 분할하고, 각 구간에 속한 캔들의 거래량 누적합을 구해 최대 거래량이 쌓인 구간의 중간 가격을 POC 가격(\(Price_{POC}\))으로 산출합니다. **(비동기 멀티 티커 환경에서의 CPU 병목을 방지하기 위해 Pandas 및 NumPy `bincount`, `argmax` 벡터화 연산으로 일괄 연산되도록 구현되었습니다.)**
* **차단 제약 조건**:
