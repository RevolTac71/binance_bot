...
class ParameterTracker:
    ...
    async def save_parameters_to_db(self, symbol: str = "ALL") -> Optional[int]:
        ...
        try:
            async with AsyncSessionLocal() as session:
                ...
                if existing:
                    logger.debug(
                        f"[ParameterTracker] 동일 파라미터 셋 이미 존재: {self.parameter_hash[:16]}..."
                    )
                    return existing.id
                ...
                new_param = StrategyParameters(
                    ...
                    strategy_version=self.current_parameters.get(
            ...
            if "does not exist" in str(e) or "UndefinedTableError" in str(e):
                logger.warning(
                    f"[ParameterTracker] 'strategy_parameters' 테이블이 아직 존재하지 않아 파라미터 저장을 건너뜁니다."
                )
            else:
    ...
    async def update_performance_metrics(
        ...
        try:
            ...
            if "does not exist" in str(e) or "UndefinedTableError" in str(e):
                logger.debug(
                    f"[ParameterTracker] 테이블 미존재로 성과 지표 업데이트 건너뜀."
                )
            else:
